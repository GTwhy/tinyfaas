...
int query_function(char * func_server_ip, uint16_t func_server_port, int app_id, int func_id)
{
	...
	switch (resp_msg->state) {
		case QUERY_FUNCTION_ACTIVE:
			printf("function state : ACTIVE\n");
			break;
		case QUERY_FUNCTION_STOP:
			printf("function state : STOP\n");
			break;
		case QUERY_FUNCTION_DEAD:
			printf("function state : DEAD\n");
			break;
	}
	free(resp_msg);
	close(sockfd);
